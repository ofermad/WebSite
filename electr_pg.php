...
<html>
 <head>
  ...
  <link rel="stylesheet" href="electr_pg.css">
 ...
 <body>
	...
	<div class="sort">
		<div>Отсортировать по названию: <a href="electr_pg.php?sort=1">↑</a> <a href="electr_pg.php?sort=2">↓</a></div>
		<div>Отсортировать по цене: <a href="electr_pg.php?sort=3">↑</a> <a href="electr_pg.php?sort=4">↓</a></div>
		<div>Отсортировать по количеству: <a href="electr_pg.php?sort=5">↑</a> <a href="electr_pg.php?sort=6">↓</a></div>
		
		<div><a href="electr_pg.php">Сбросить</a></div>
	</div>
	
	<?php
		$sql = mysqli_query($des, 'SELECT * FROM `car_electronics`'.$s);
		echo '<div class="product-feed">';
		
		while($result = mysqli_fetch_array($sql))
		{
			echo '<div class="product">';
			echo '<img src="Pictures_for_site\car_electronics\\'.$result['img_path'].'" onclick="showModal('.$result['id_electr'].')">';
			echo '<h3 class="prod_nm" onclick="showModal('.$result['id_electr'].')">'.$result['electr_name'].' </h3>';
			echo '<h4 class=prod_price>'.'Цена: '.$result['price'].' руб.</h4>';
			//echo '<p>В наличии: '.$result['quantity'].' шт.</p>';
			echo '<button onclick="addToCart(\''.htmlspecialchars($result['electr_name']).'\', '.$result['price'].')">В корзину</button>';
			echo '</div>';
			
			echo '<div id="'.$result['id_electr'].'" class="modal">';
			echo '<div class="modal-content">';
			echo '<span class="close_modal" onclick="closeModal('.$result['id_electr'].')">&times;</span>';
			echo '<img src="Pictures_for_site\car_electronics\\'.$result['img_path'].'">';
			echo '<h2>'.$result['electr_name'].'</h2>';
